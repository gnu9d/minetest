...
class ClInvRef : public ModApiBase {
private:
	InventoryLocation m_loc;
	...
	static ClInvRef *checkobject(lua_State *L, int narg);

	static Inventory* getinv(lua_State *L, ClInvRef *ref);

	static InventoryList* getlist(lua_State *L, ClInvRef *ref,
			const char *listname);

	static void reportInventoryChange(lua_State *L, ClInvRef *ref);
...
public:
	ClInvRef(const InventoryLocation &loc);
	...
	// Not callable from Lua; all references are created on the C side.
	static void create(lua_State *L, const InventoryLocation &loc);
	static void createGCAOPlayer(lua_State *L, GenericCAO *player);
	static void createNodeMeta(lua_State *L, v3s16 p);
	static void Register(lua_State *L);
};

class ModApiClInventory : public ModApiBase {
private:
	static int l_create_detached_inventory_raw(lua_State *L);
